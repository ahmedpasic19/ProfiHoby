...
export default async function handler(
  ...
  if (req.method === 'POST') {
    if (!req.query.id)
      return res.status(400).json({ message: 'Listing id title required' })
    ...
    // const entity = cities?.data?.data?.find((entity) => entity.code === 'FBiH')
    ...
    if (!category)
      return res.status(500).json({ message: 'Cannot find suggested category' })
    ...
    axios
      ...
      .catch((error: any) => {
        ...
        // console.log(error.response.data.error._debug)
      })
